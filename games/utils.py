...
def search_games_on_retrogames(query, max_results=5):
    ...
    try:
        ...
        search_url = f"https://www.retrogames.cc/search?q={query}"
        ...
        # O retrogames.cc pode ter diferentes estruturas HTML, vamos tentar várias
        ...
        for link in game_links:
            ...
            # Tentar obter URL do embed (geralmente está na URL do jogo)
            embed_url = None
            if '/embed/' in game_url:
                embed_url = game_url
            elif '/play/' in game_url:
                # Converter /play/ para /embed/
                embed_url = game_url.replace('/play/', '/embed/')
            else:
                # Tentar construir URL de embed baseado na URL do jogo
                game_id = game_url.split('/')[-1].split('-')[0]
                if game_id.isdigit():
                    embed_url = f"https://www.retrogames.cc/embed/{game_id}.html"
